		...
		if (pecaSelecionada.equals("R")) {
			...
			if (pecaCheque.toString().equals("C") || pecaCheque.toString().equals("P")) {
				if (mat[pecaCheque.getPosicao(pecaCheque).getLinha()][pecaCheque.getPosicao(pecaCheque).getColuna()]) {
					...
					throw new ExceptionXadrez(s + " em " + posicaoOrigem + " nao consegue defender o cheque - Press Enter");
				}
			}
			...
			aux = getPecaCheque(jogadorAtual).movimentosHipoteticos(posicaoRei);			    
				...
				if (matrizTotalmenteFalsa(mat)) {
					String s = pecaSelecionada.toString();
						switch (s) {
							case "D": s = "Dama";
							break;
							case "T": s = "Torre";
						...
						throw new ExceptionXadrez(s + " em " + posicaoOrigem + " nao pode defender o cheque - Press Enter");
				}
			}
			}
			} else {
				...
				Peca p = pecaCravada(posicao, posicaoRei, pecasAdversarias, aux);
					if (p != null) {
						...
						if (matrizTotalmenteFalsa(mat)) {
							...
							throw new ExceptionXadrez("A peca " + s + " em " + posicaoOrigem + " esta cravada. Impossível mexer - Press Enter");
						}
						return mat;						
					}			 
				return mat;
