...
public class PartidaXadrez {
	...
	public boolean[][] movimentosPossiveis(PosicaoXadrez posicaoOrigem) {
		...
		List<Peca> pecasAdversarias = 
			pecasNoTabuleiro.stream().filter(x -> ((PecaXadrez)x).getCor() != jogadorAtual).collect(Collectors.toList()); 
		...
		if (tabuleiro.peca(posicao).toString().equals("R")) {
			...
			pecasAdversarias = pecasAdversarias.stream().filter(x -> 
								(x.toString().equals("T") ||
								x.toString().equals("B") ||
								x.toString().equals("D"))).collect(Collectors.toList());
			if (reiEstaEmCheque(jogadorAtual)) {
			//se o rei está em cheque duplo, e a peça selecionada não é o rei, nenhuma jogada é possível.
		      if (reiEstaEmChequeDuplo(jogadorAtual) && !tabuleiro.peca(posicao).toString().equals("R")) {
			...
			pecasAdversarias = pecasAdversarias.stream().filter(x -> 
								(x != pecaCheque)).collect(Collectors.toList());
			...
			aux = getPecaCheque(jogadorAtual).movimentosHipoteticos(posicaoRei);
				...
				Peca p = pecaCravada(posicao, posicaoRei, pecasAdversarias, aux);
					if (p != null) {
					// eu acho que nao precisa, a matriz aux é alterada dentro do método peca Cravada
	...
	private boolean reiEstaEmChequeDuplo(Cor color) {
...
*/	
		colocarNovaPeca('a', 4, new Bispo(tabuleiro, Cor.BRANCA));
		...
		colocarNovaPeca('b',6, new Dama(tabuleiro, Cor.BRANCA));
        //colocarNovaPeca(pI[3],linhasAleatorias[5], new Dama(tabuleiro, Cor.BRANCA));
        //colocarNovaPeca(pI[4], linhasAleatorias[4], new Rei(tabuleiro, Cor.BRANCA, this));

		colocarNovaPeca('b', 7, new Rei(tabuleiro, Cor.BRANCA, this));
...
*/
	   colocarNovaPeca('b', 2, new Dama(tabuleiro, Cor.PRETA));
		colocarNovaPeca('f', 6, new Bispo(tabuleiro, Cor.PRETA));

        colocarNovaPeca('a', 1, new Rei(tabuleiro, Cor.PRETA, this));
