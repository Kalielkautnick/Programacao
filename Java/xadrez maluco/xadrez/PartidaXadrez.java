...
public class PartidaXadrez {
	...
	public boolean[][] movimentosPossiveis(PosicaoXadrez sourcePosition) {
		...
		if (reiEstaEmCheque(jogadorAtual) || tabuleiro.peca(posicao).toString().equals("R")) {
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
					boolean[][] aux = new boolean[15][15];
                 		if (tabuleiro.peca(i,j) != null) {
						aux = tabuleiro.peca(i,j).movimentosPossiveis();
				 		PecaXadrez pecaAdversaria = (PecaXadrez)tabuleiro.peca(i,j);
				 				if (tabuleiro.peca(i,j) != tabuleiro.peca(posicao) && pecaAdversaria != null && pecaAdversaria.getCor() != jogadorAtual) {
									for (int k = 0; k < 8; k++) {
										for (int m = 0; m < 8; m++) {
											if (aux[k][m]) {
											mat[k][m] = false;
